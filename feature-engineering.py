...
def add_target_encoding(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["store_mean"]           = df.groupby("Store")["Weekly_Sales"].transform(
                                     lambda x: x.shift(1).expanding().mean())
    df["dept_mean"]            = df.groupby("Dept")["Weekly_Sales"].transform(
                                     lambda x: x.shift(1).expanding().mean())
    df["type_mean"]            = df.groupby("Type")["Weekly_Sales"].transform(
                                     lambda x: x.shift(1).expanding().mean())
    df["store_dept_mean"]      = df.groupby(["Store", "Dept"])["Weekly_Sales"].transform(
                                     lambda x: x.shift(1).expanding().mean())
    df["store_dept_median"]    = df.groupby(["Store", "Dept"])["Weekly_Sales"].transform(
                                     lambda x: x.shift(1).expanding().median())
    df["store_dept_std"]       = df.groupby(["Store", "Dept"])["Weekly_Sales"].transform(
                                     lambda x: x.shift(1).expanding().std())
    df["dept_week_mean"]       = df.groupby(["Dept", "Week"])["Weekly_Sales"].transform(
                                     lambda x: x.shift(1).expanding().mean())
    df["store_week_mean"]      = df.groupby(["Store", "Week"])["Weekly_Sales"].transform(
                                     lambda x: x.shift(1).expanding().mean())
    df["store_dept_week_mean"] = df.groupby(["Store", "Dept", "Week"])["Weekly_Sales"].transform(
                                     lambda x: x.shift(1).expanding().mean())
...
def add_seasonal_and_trend_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dept_annual          = df.groupby("Dept")["Weekly_Sales"].transform(
                               lambda x: x.shift(1).expanding().mean())
    dept_week_expanding  = df.groupby(["Dept", "Week"])["Weekly_Sales"].transform(
                               lambda x: x.shift(1).expanding().mean())
    df["dept_seasonal_idx"] = dept_week_expanding / (dept_annual + 1e-8)

    store_annual         = df.groupby("Store")["Weekly_Sales"].transform(
                               lambda x: x.shift(1).expanding().mean())
    store_week_expanding = df.groupby(["Store", "Week"])["Weekly_Sales"].transform(
                               lambda x: x.shift(1).expanding().mean())
